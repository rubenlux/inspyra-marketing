...
export class BusinessContextEngine {
    process(input) {
        ...
        const classificationCoverage = this.determineClassificationCoverage(businessClassification);
        // ========================================================================
        ...
        if (input.signals && Object.keys(input.signals).length > 0) {
            ...
            const navSections = (signals.mainNavSections || []).map((s) => s.toLowerCase());
        ...
        const metadata = {
            ...
            dataIntegrity: this.determineDataIntegrity(businessClassification, websiteAnalysis, classificationCoverage),
    ...
    classifySubindustriesFromRubro(rubro) {
        const subindustries = [];
    ...
    // ========================================================================
    analyzeWebsite(rubro, navSections, signals) {
        const capabilities = {
            ...
            hasLeadCapture: signals.hasLeadForm || signals.hasContactForm || null,
    ...
    // ==========================================================================
    detectPatterns(rubro, navSections, signals) {
        const patterns = [];
        // PATTERN 1: Invitation without mechanism
        const hasReservaNAv = navSections.some((nav) => nav.includes("reserva") ||
        ...
        if (hasReservaNAv &&
            ...
            patterns.push({
                ...
                description: "Nav invites to book/reserve but no online booking, contact form, or phone visible",
                signals: [
    ...
    // ==========================================================================
    classifyBusinessModels(signals) {
        const models = [];
        if (signals.hasEcommerce) {
            models.push({
                model: "ECOMMERCE",
        ...
        if (signals.hasPhone || signals.hasContactForm) {
            models.push({
                ...
                signalCode: "hasPhone=true OR hasContactForm=true",
            });
        }
        return models;
    }
    ...
    // METADATA HELPERS
    // ==========================================================================
    determineClassificationCoverage(classification) {
    ...
    determineDataIntegrity(classification, analysis, coverage) {
    ...
    // NOTES GENERATION
    // ==========================================================================
    generateNotes(classification, analysis) {
        const notes = [];
        if (analysis === null) {
            ...
            const unverifiedPatterns = analysis.observedPatterns.filter((p) => p.status === "UNVERIFIED");
            if (unverifiedPatterns.length > 0) {
                notes.push(`${unverifiedPatterns.length} pattern(s) require verification via link crawling`);
            }
        }
        return notes;
    }
