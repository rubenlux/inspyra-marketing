import { BusinessContext, ValidationResult, ValidationError } from "./types";
...
export class BusinessContextValidator {
  ...
  private validateMetadata(context: BusinessContext, errors: ValidationError[]): void {
  ...
  private validateBusinessClassification(context: BusinessContext, errors: ValidationError[]): void {
    const { industry, subindustries } = context.businessClassification;

    // REGLA 2: Accommodation ELIMINADO
    const hasAccommodation = subindustries.some(
      (s) => "value" in s && s.value === "Accommodation"
    );
    if (hasAccommodation) {
      errors.push({
    ...
    for (let i = 0; i < subindustries.length; i++) {
      ...
      if ("source" in sub && sub.source !== null) {
        ...
        this.validateClassificationWithEvidence(
          sub,
          `businessClassification.subindustries[${i}]`,
          errors
        );
    ...
    const wineClassified = subindustries.find(
      (s) => "value" in s && s.value === "Wine/Beverages" && "source" in s && s.source !== null
    ) as any;
  ...
  private validateWebsiteAnalysis(context: BusinessContext, errors: ValidationError[]): void {
  ...
  private validateClassificationWithEvidence(
    classification: any,
    fieldPath: string,
  ...
  private validateNoBusinessInferences(context: BusinessContext, errors: ValidationError[]): void {
    ...
    for (const inference of prohibitedInferences) {
      if (contextJson.includes(inference.toLowerCase())) {
        ...
        const inClassification = context.businessClassification.subindustries.some(
          (s) => "value" in s && s.value.toLowerCase().includes(inference)
        );
        if (inClassification) {
          errors.push({
            field: "businessClassification",
   ...
   */
  validateNoContradictions(context: BusinessContext): ValidationResult {
    const errors: ValidationError[] = [];
